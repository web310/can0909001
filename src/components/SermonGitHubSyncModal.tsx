...
`;
  ...
  const handleDirectGitHubPush = async () => {
    ...
    try {
      ...
      // Also sync public/canaan_master_data.json to GitHub repository
      try {
        const masterRes = await fetch(`/canaan_master_data.json?t=${Date.now()}`);
        if (masterRes.ok) {
          const masterJson = await masterRes.json();
          if (masterJson && masterJson.data) {
            masterJson.data.sermons = sermons;
            masterJson.exportedAt = new Date().toISOString();
            const masterContent = btoa(unescape(encodeURIComponent(JSON.stringify(masterJson, null, 2))));
            let masterSha: string | undefined;
            const getMaster = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/public/canaan_master_data.json?ref=${targetBranch}`, {
              headers: { Authorization: authHeader, Accept: 'application/vnd.github+json' }
            });
            if (getMaster.ok) {
              const mData = await getMaster.json();
              masterSha = mData.sha;
            }
            await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/public/canaan_master_data.json`, {
              ...
              body: JSON.stringify({
                message: `chore(data): update master snapshot with latest sermons (${sermons.length})`,
