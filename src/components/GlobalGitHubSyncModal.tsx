...
import { loadAndSyncSermons } from '../utils/sermonStorage';
...
export const GlobalGitHubSyncModal: React.FC<GlobalGitHubSyncModalProps> = ({
  ...
  const loadSermonsFromStorage = () => {
    try {
      setAllSermons(loadAndSyncSermons());
      return;
    } catch {}
...
`;
  ...
  const generateCloudflareSermonsJs = (): string => {
    return `// Cloudflare Pages Function: /api/sermons
const DEFAULT_SERMONS = ${JSON.stringify(allSermons, null, 2)};
...
`;
  ...
  const handleDirectGitHubPushAll = async () => {
    ...
    const filesToSync = [
      ...
      { path: 'functions/api/sermons.js', content: generateCloudflareSermonsJs(), nameZh: 'Cloudflare Pages 講道 API 邊緣函數' },
